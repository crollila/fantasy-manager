...
export function Home({api,onAdvanced}:{api:Request;onAdvanced:()=>void}){
 ...
 <div className="home-content"><div className="home-tabs">{['My teams','NFL games','Accuracy','Learning','Data & evidence'].map(v=><button key={v} className={view===v?'selected':''} onClick={()=>setView(v)}>{v}</button>)}</div><header><div><div className="eyebrow">FANTASY FOOTBALL</div><h1>{view==='My teams'?'Your weekly lineup':view==='NFL games'?'NFL game forecasts':view==='Accuracy'?'Forecast accuracy':view==='Learning'?'Learning from every game':'Data & methodology'}</h1><p className="muted">{view==='My teams'?'Your teams, injury updates and a clear lineup recommendation.':'Updated on app open. Every forecast is saved before its game begins.'}</p></div><button className="primary" disabled={busy} onClick={()=>{setAdding(!adding);setView('My teams');}}>Manage ESPN</button></header>
 ...
 {view!=='My teams'&&<Intelligence api={api} report={research} view={view} busy={researchBusy} onRefresh={refreshResearch}/>}
 {view==='My teams'&&<>
