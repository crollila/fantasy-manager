...
Version 0.6.3 makes the whole prediction loop automatic and adds a validated player projection model: the NFL engine refits itself after each completed week, every player with an upcoming game is projected and archived before kickoff, results are graded automatically, and weekly projections now use snap share, red-zone role and expected passing volume (walk-forward MAE 4.59 → 4.42 on 17,110 held-out player-weeks). Version 0.6.2 put each team’s logo on the NFL game cards, bundled with the app so they appear offline. Version 0.6.1 fixed the win probability shown beside a game pick when the two data sources spell a team differently (the Los Angeles Rams are LA in nflverse and LAR on the scoreboard), which could display the opponent's probability and mis-grade a completed Rams home game. Version 0.6.0 added the NFL game forecasting engine described below: NFL picks now come from a walk-forward-validated ensemble trained on 27 seasons of play-by-play data, with calibrated win probabilities, score distributions and drivers, and the trained champion ships inside the installer. Version 0.5.0 introduced the standalone Windows application and installer. The simpler home screen puts your teams first, with one **Find my best lineup** action and draft/research features under **Advanced tools**. Connect public or private leagues through an isolated ESPN sign-in window; no copying league IDs or cookies is required. Sign-in is performed by you, and ESPN account access is not bundled with the app. The installer includes Python, the analysis engine, and the desktop UI.
